...
| `TeamRequestList` | `leave/TeamRequestList.jsx` | The manager/HR approvals list — approve/reject on `SUBMITTED` rows (reject expands an inline optional-comment box), plus an HR-only override control on already-decided rows. Same "Details" action as `MyLeaveRequestList`. Each row shows the employee's `RoleBadge` next to their name (HR's list spans every role, so it isn't otherwise obvious) and labeled `Button`s rather than icon-only actions — the row wraps (`flex-wrap`) rather than staying rigidly single-line once there are several labeled actions plus two badges. | `requests`, `canOverride`, `onChanged` |
| `RequestDetailModal` | `leave/RequestDetailModal.jsx` | The single "Details" destination for both request lists above — everything about one request (employee, dates, reason, decision, full audit trail) plus its attached document, if any, via `DocumentPreview`. Replaces the old separate `AuditTrail` modal (retired) and the old "View document" new-tab action. Fetches the audit trail and, if `request.has_document`, the signed document URL lazily on open. | `request` (the full row, not just an id — the caller's list already has it), `open`, `onClose` |
| `DocumentPreview` | `leave/DocumentPreview.jsx` | Inline document preview (`<img>` for jpg/png, `<iframe>` for pdf) with a "view full screen" button that opens a lightbox (its own `document.body` portal, like `Modal`) showing the document larger. The lightbox's Escape handler lives on the lightbox element itself and calls `stopPropagation`, not a `document`-level listener like `Modal` uses — otherwise one Escape press while the lightbox is open would also close the surrounding `RequestDetailModal`, since both listeners would be on `document` and both would see the same keypress. | `url`, `mimeType`, `filename` |
