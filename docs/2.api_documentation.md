...
```
`decided_by_first_name`/`decided_by_last_name` are resolved server-side from `decided_by` (null until the request is decided) so the UI never has to show a raw user id. `has_document` (FR-012) tells the UI whether to show a "view document" action, without a client needing to call `GET /:id/document` on every row just to find out.
