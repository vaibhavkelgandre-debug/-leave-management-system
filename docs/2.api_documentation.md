...
**Auth**: `HR_ADMIN` at the route level, but when `:id` is itself an `HR_ADMIN`, only the specific HR admin who created them (`invited_by`) may actually change it — any other HR admin gets a `403`, same as a peer manager trying to edit someone outside their team. A root `HR_ADMIN` (`invited_by: null`) can't have this changed by anyone through this endpoint. Reassigning a `MANAGER`/`EMPLOYEE`'s manager is unaffected — any `HR_ADMIN` still can, as before.
...
**Errors**: `400` self-assignment, or the chosen manager's role doesn't satisfy the hierarchy rule (e.g. a `MANAGER` picked as an `HR_ADMIN`'s manager) · `403` caller isn't `HR_ADMIN` at all, **or** is HR but isn't the specific HR admin who created this `HR_ADMIN` target · `404` user not found · `409` would create a reporting cycle · `422` validation.
...
- Document upload for leave requests (FR-012) — explicitly deferred pending a decision on file storage.
- HR reporting/CSV export (Module 4 of the functional requirements).
