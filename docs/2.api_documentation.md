...
`payPeriod` must be a **fully completed past month** — its last day must already be before today. A period that hasn't started yet, or has started but not finished (this month, mid-month), both reject with `400`: HR can only run "last month's numbers this month," never a preview of the still-running current month. Note this still returns a full `"ok"` row for an employee/period that already has a committed, `ACTIVE` slip — the "already generated" guard below only blocks `/confirm`, not this preview.
