...
```json
{
  ...
  "employee_first_name": "...", "employee_last_name": "...", "employee_manager_id": "...",
  "created_at": "...", "updated_at": "..."
}
```
`decided_by_first_name`/`decided_by_last_name` are resolved server-side from `decided_by` (null until the request is decided) so the UI never has to show a raw user id.
...
**Errors**: `400` leave type inactive/not found, range has zero working days, or the balance would go negative and the leave type doesn't allow it · `409` overlaps an existing `SUBMITTED`/`APPROVED` request of the caller's · `422` validation.
...
Requests the caller can act on: **direct reports only** for a `MANAGER` (not the full reporting subtree — approval authority belongs to the direct manager or their delegate, not a skip-level manager), everyone's for `HR_ADMIN`.

**Auth**: `MANAGER` or `HR_ADMIN`.

**Response** `200` — array of the row shape above.

**Errors**: `403` caller is an `EMPLOYEE`.
...
**Auth**: the request's direct manager, an active delegate for that manager, or `HR_ADMIN` — **never** the request's own employee, regardless of role (this is what the "employee cannot approve their own request" rule enforces).
...
**Errors**: `403` caller is the request's own employee · `404` caller has no relationship to this request · `409` request isn't `SUBMITTED` · `422` validation.
...
**Auth**: `HR_ADMIN` only (a plain role check — override is HR's blanket authority, not scoped to a team).
...
**Errors**: `403` caller isn't `HR_ADMIN` · `409` `toStatus: "APPROVED"` on a request that isn't `REJECTED` (or vice versa) · `422` validation.

---

## Delegations (`/api/delegations`)

Every route below requires `requireAuth` **and** `MANAGER` — nominating a delegate, and listing your own, are both manager-only (FR-020). `manager_id` is always the caller, never client-supplied.
