...
> 📅 Reflects the codebase as of migration `036_alter_notifications_add_profile_created.sql` (018 briefly widened `oauth_accounts.provider` to allow `GITHUB`; 019 reverted it after GitHub login was removed — net schema is unchanged, still `GOOGLE` only. 020–031 are Module 5, added beyond the original brief and reworked once the real spreadsheet-based requirements arrived: `020`/`022`/`031` are `users` profile fields (022 replaced 020's initial 10-column sketch with the fuller ~24-column set matching the source spreadsheet; 031 added the `profile_send_back_reason`/`_by`/`_at` trio so HR's send-back always carries an explanation), `021`/`026`/`030`/`035` are `salary_slips`/`salary_slip_revisions` (026 reworked their columns from generic CSV figures to salary-structure components + LOP; 030 added a `status`/void-tracking set of columns so HR can soft-delete a slip generated for the wrong pay period; 035 added `total_leave_days`/`payable_days` alongside a completed-month-only, joining-date-aware generation rule), `023`/`027`/`028`/`029` are `employee_documents` (027 replaced its original two document types with PAN card/Aadhar card/bank passbook; 028 added `OTHER` for any number of additional self-named documents; 029 added a 4th required type, `OFFER_LETTER`), `024` is `salary_structures`/`salary_structure_revisions`, `025` adds `leave_types.counts_as_lop`, `032` adds the in-app `notifications` table, `033` expands its `type`/`entity_type` CHECK constraints with 9 more role-driven scenarios, `034` seeds the `SUPER_ADMIN` role row, `036` adds a 17th notification type, `PROFILE_CREATED`, telling a new employee to complete their profile right after they accept their invite).
